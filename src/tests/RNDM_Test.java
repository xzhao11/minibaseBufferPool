...
class BMDriver extends TestDriver implements GlobalConst {
  ...
  public boolean runTests () {
    ...
    try {
      SystemDefs sysdef = new SystemDefs(dbpath, NUMBUF+20, NUMBUF, "RNDM");
    } catch (Exception e) {
  ...
  protected boolean test2 () {
    ...
    for (pid.pid = firstPid.pid + 1, lastPid.pid = firstPid.pid + numPages - 1;
          status == OK && pid.pid < lastPid.pid; pid.pid = pid.pid + 1 ) {
  ...
  protected String testName () {
    return "Buffer Management - RNDM";
  }
}

public class RNDM_Test {
   ...
   public static void main (String argv[]) {
     ...
     if (dbstatus != true) {
       System.err.println ("Error encountered during buffer manager RNDM tests:\n");
       Runtime.getRuntime().exit(1);
     }
